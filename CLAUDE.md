...
הרפרנס: `mercazharav.org.il`. **רק הפריסה וההיררכיה אומצו — הפלטה נשארה "חותם"
...
2. **כל סקשן תוכן הוא כרטיס לבן** — `section.panel > .container` מקבל `--surface`,
   גבול `--line-2`, ו-`box-shadow: var(--sh-1)`. הסקשן עצמו הופך למרזב: `background: none`.
3. **קו זהב חוסם את ראש הכרטיס** — `border-top: 3px solid var(--gold-500)`.
   זה מחליף את `.sec-title::after` שרץ מהכותרת החוצה; בתוך פאנל הוא `display:none`.
4. **ראש הסקשן הוא שורה אחת** — גריד דו-טורי (`auto 1fr`). כותרת ב-`grid-column:1`
...
5. **פינות 4px** — `--r-panel`. הפאנל מאפס `--r-xs/sm/md/lg` ל-4px לכל מה שבתוכו,
   כך שכל הגאומטריה אחידה. `--r-pill` נשאר לכפתורים.
6. **צפיפות** — `--panel-gap: var(--s5)` (24px) בין כרטיסים במקום 96px אוויר.
